...
void add_sphere( struct matrix * edges, double cx, double cy, double cz, double r, int step ) {
    ...
    for (int i = 0; i < sphere->lastcol; i++) add_edge(edges, sphere->m[0][i], sphere->m[1][i], sphere->m[2][i], sphere->m[0][i], sphere->m[1][i], sphere->m[2][i]);
    free_matrix(sphere);
}
...
void add_torus( struct matrix * edges, double cx, double cy, double cz, double r1, double r2, int step ) {
    ...
    for (int i = 0; i < torus->lastcol; i++) add_edge(edges, torus->m[0][i], torus->m[1][i], torus->m[2][i], torus->m[0][i] + 1, torus->m[1][i] + 1, torus->m[2][i] + 1);
    free_matrix(torus);
}
...
struct matrix * generate_torus( double cx, double cy, double cz, double r1, double r2, int step ) {
    ...
    for (int i = 0; i < step; i++) {
        for (int j = 0; j < step; j++) {
            double phi = 2 * M_PI * i / step;
            double theta = 2 * M_PI * j / step;
            double x = cx + (cos(phi) * (r1 * cos(theta) + r2));
            double y = cy + (r1 * sin(theta));
            double z = cz + (-sin(phi) * (r1 * cos(theta) + r2));
